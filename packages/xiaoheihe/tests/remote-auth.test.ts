...
describe("小黑盒远程登录态(配置中心加密域)", () => {
  ...
  it("remote 登录态:存 → 新机还原 → client 构造可用(带 cookie)", async () => {
    const dirA = mkdtempSync(join(tmpdir(), "xhh-remote-a-"));
    const dirB = mkdtempSync(join(tmpdir(), "xhh-remote-b-"));

    // 电脑 A:登录态存本地 + 远程(加密)
    const storeA = new AuthStore({ platform: "xiaoheihe", path: join(dirA, "auth.json"), remote });
    await storeA.save({
      platform: "xiaoheihe",
      credentials: { cookie: "x_xhh_tokenid=mock-token-999" },
      savedAt: new Date().toISOString(),
    });

    // 电脑 B(新机,无本地缓存):load() 从远程还原并回写本地
    const storeB = new AuthStore({ platform: "xiaoheihe", path: join(dirB, "auth.json"), remote });
    const restored = await storeB.load();
    expect(restored?.credentials).toMatchObject({ cookie: "x_xhh_tokenid=mock-token-999" });

    // 构造 client(带 remote):能从还原后的本地缓存加载 cookie,不抛错
    const client = createXiaoheiheClient({ remote });
  ...
  it("远程不可达时降级:client 构造仍可用(本地缓存)", async () => {
    ...
    await store.save({
      platform: "xiaoheihe",
      credentials: { cookie: "x_xhh_tokenid=local-only" },
      savedAt: new Date().toISOString(),
    });
    // 远程失败不抛,本地可用,client 构造正常
    const client = createXiaoheiheClient({ remote: badNs });
    expect(client).toBeDefined();
  });
});
