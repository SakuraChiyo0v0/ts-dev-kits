...
export function createLlmProxy(options: LlmProxyOptions): Server {
      ...
      try {
        ...
        const result = await client.generateImage({
          ...
          ...(typeof body.response_format === "string"
            ? { responseFormat: body.response_format as "b64_json" | "url" }
            : {}),
        });
...
function normalizeRequest(body: Record<string, unknown>, defaultModel?: string): ChatRequest {
  ...
  if (Array.isArray(body.tools)) {
    result.tools = body.tools as NonNullable<ChatRequest["tools"]>;
  }
  if (body.tool_choice !== undefined) {
    result.toolChoice = body.tool_choice as NonNullable<ChatRequest["toolChoice"]>;
  }
  return result;
}

/** 把统一响应转成 OpenAI 线上格式。 */
function toWireResponse(result: Awaited<ReturnType<LLMClient["chat"]>>): Record<string, unknown> {
  return {
    id: result.id,
...
function statusForCode(code: LlmError["code"]): number {
  switch (code) {
