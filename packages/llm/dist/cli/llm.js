#!/usr/bin/env node
import { CliError, getBool, getString, handleCliError, outputJson, outputText, parseArgs, printHelp, requireString, } from "@amechan/cli-utils";
import { createLlmClient, createProviderAdapter, listProviders, openaiAdapter, } from "../index.js";
...
function makeAdapter(provider, apiKey) {
    ...
    try {
        ...
        throw new CliError(`Unknown provider "${provider}". Native: openai/anthropic/gemini/azure. Registered: ${listProviders().join(", ")}`);
    }
}
...
async function main() {
    ...
    switch (command) {
        ...
        case "chat": {
            ...
            if (apiKey === "") {
                throw new CliError(`No API key for provider "${provider}". Pass --api-key or set ${provider.toUpperCase()}_API_KEY`);
            }
