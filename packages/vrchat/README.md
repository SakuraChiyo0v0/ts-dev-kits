...
const remote = createWebdavConfigCenter().namespace("auth", { encrypt: true });
...
| `remote` | `AuthRemoteStore` | 远程登录态命名空间(配置中心加密域);登录态双写本地+远程,远程不可达自动降级本地,新机可用 `AuthStore.load()` 还原 |
