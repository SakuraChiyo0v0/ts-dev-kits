...
describe("网易云远程登录态(配置中心加密域)", () => {
  ...
  it("remote 登录态:存 → 新机还原 → client 构造可用(带 cookie)", async () => {
    const dirA = mkdtempSync(join(tmpdir(), "netease-remote-a-"));
    const dirB = mkdtempSync(join(tmpdir(), "netease-remote-b-"));

    // 电脑 A:登录态存本地 + 远程(加密)
    const storeA = new AuthStore({ platform: "netease-music", path: join(dirA, "auth.json"), remote });
    await storeA.save({
      platform: "netease-music",
      credentials: { cookies: "MUSIC_U=mock-token-999" },
      savedAt: new Date().toISOString(),
    });

    // 电脑 B(新机,无本地缓存):load() 从远程还原并回写本地
    const storeB = new AuthStore({ platform: "netease-music", path: join(dirB, "auth.json"), remote });
    const restored = await storeB.load();
    expect(restored?.credentials).toMatchObject({ cookies: "MUSIC_U=mock-token-999" });
  ...
  it("远程不可达时降级:client 构造仍可用(本地缓存)", async () => {
    const dir = mkdtempSync(join(tmpdir(), "netease-remote-fail-"));
    ...
    const store = new AuthStore({ platform: "netease-music", path: join(dir, "auth.json"), remote: badNs });
    await store.save({
      platform: "netease-music",
      credentials: { cookies: "MUSIC_U=local-only" },
      savedAt: new Date().toISOString(),
    });
    // 远程失败不抛,本地可用,client 构造正常
    const client = createNeteaseClient({ remote: badNs });
    expect(client).toBeDefined();
  });
});
