...
describe("Steam 远程登录态(配置中心加密域)", () => {
  ...
  it("remote 登录态:存 → 新机还原 → client 构造可用(带 cookie)", async () => {
    const dirA = mkdtempSync(join(tmpdir(), "steam-remote-a-"));
    const dirB = mkdtempSync(join(tmpdir(), "steam-remote-b-"));

    // 电脑 A:登录态存本地 + 远程(加密)
    const storeA = new AuthStore({ platform: "steam", path: join(dirA, "auth.json"), remote });
    await storeA.save({
      platform: "steam",
      credentials: { cookies: "steamLoginSecure=mock-token-999; sessionid=abc123" },
      savedAt: new Date().toISOString(),
    });

    // 电脑 B(新机,无本地缓存):load() 从远程还原并回写本地
    const storeB = new AuthStore({ platform: "steam", path: join(dirB, "auth.json"), remote });
    const restored = await storeB.load();
    expect(restored?.credentials).toMatchObject({
      cookies: "steamLoginSecure=mock-token-999; sessionid=abc123",
    });

    // 构造 client(带 remote):能从还原后的本地缓存加载 cookie,不抛错
    const client = createSteamClient({ remote });
    expect(client).toBeDefined();
  });

  it("远程不可达时降级:client 构造仍可用(本地缓存)", async () => {
    const dir = mkdtempSync(join(tmpdir(), "steam-remote-fail-"));
    ...
    const store = new AuthStore({ platform: "steam", path: join(dir, "auth.json"), remote: badNs });
    await store.save({
      platform: "steam",
      credentials: { cookies: "steamLoginSecure=local-only" },
      savedAt: new Date().toISOString(),
    });
    // 远程失败不抛,本地可用,client 构造正常
    const client = createSteamClient({ remote: badNs });
    expect(client).toBeDefined();
  });
});
