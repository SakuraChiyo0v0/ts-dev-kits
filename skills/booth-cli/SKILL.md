...
```bash
sc-booth help    # 查看命令与选项
which sc-booth   # 确认已安装
```
...
```bash
sc-booth login              # 自动浏览器登录:弹出独立 Chrome,登录后自动捕获会话(零复制粘贴)
sc-booth login --reuse      # 复用日常浏览器登录态(需先关闭 Chrome;免重新输账号密码)
sc-booth login --manual     # 手动模式:F12 复制 Cookie 头粘贴
sc-booth status             # 查看登录状态
...
```bash
sc-booth parse "https://booth.pm/ja/items/12345"
sc-booth parse 12345
# 输出商品 id / 标题 / 价格(日元,0=免费)/ 店铺 / 是否已拥有

sc-booth parse "https://booth.pm/ja/items/12345" --detail
# 额外输出:简介/正文(description)+ 全部购买项(variations,含免费/付费档)
# 购买项示例:名称 / 价格 / 是否免费 / 免费项下载直链 / variation_id
sc-booth parse "12345" --detail --no-description   # 只要购买项,不要简介(省 token)
sc-booth parse "12345" --detail --no-variations    # 只要简介,不要购买项
```
...
```bash
sc-booth claim "https://booth.pm/ja/items/12345"
# 免费商品 → claimed,自动下载到本地(默认)
# 付费商品 → paid-pending + 购物车 URL(浏览器手动加购/支付,不下载)

sc-booth claim "https://booth.pm/ja/items/1" "https://booth.pm/ja/items/2" --concurrency 2
# 批量领取,并发可配(默认 1,避免站方压力)

sc-booth claim "12345" --no-download
# 只领取不下载

sc-booth claim "12345" --output-dir ./downloads
# 指定下载目录(默认当前目录)
```
...
```bash
sc-booth download "https://s6.booth.pm/xxxx/...zip?X-Amz-..." --output-dir ./downloads
