...
```bash
sc-config setup --url http://dav.amechan.cloud/dav/ --username webdav --password "应用密码" --key "加密密钥"
sc-config status    # 查看配置状态(密码/密钥脱敏)
sc-config clear     # 清除本地全局配置
```
...
```bash
# 明文域(普通配置)
sc-config set bilibili ui --json '{"quality":80}'     # 写 /amechan/configs/bilibili/ui
sc-config get bilibili ui                              # 读
sc-config list bilibili                                # 列出该域配置名
sc-config remove bilibili ui                           # 删

# 加密域(敏感配置:登录态/cookie/密钥)加 --encrypt
sc-config set xiaoheihe auth --json '{"cookie":"SID=..."}' --encrypt   # /amechan/secrets/xiaoheihe/auth(密文)
sc-config get xiaoheihe auth --encrypt                                 # 解密读
```
