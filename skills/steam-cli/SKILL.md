...
让 AI 直接用 `amechan-steam` 命令行操作 Steam SDK(查询向):登录、资料、游戏库、成就、市场价、搜索、库存、我的挂单。**零写操作**——不提供市场买卖、交易创建/接受、好友增删等任何写命令。
...
- 私密命令(如 `my-listings`)未登录时抛 `LOGIN_REQUIRED`。
...
| `--currency <n>` | 货币代码(price,默认 1=USD;23=CNY) |
...
SDK 与 CLI **零写操作**:无市场买卖、交易创建/接受、好友增删、愿望单写。如遇 AI 提出此类操作,直接拒绝并说明边界。
