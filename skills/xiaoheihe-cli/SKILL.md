# amechan-xiaoheihe CLI

让 AI 直接用 `amechan-xiaoheihe` 命令行操作小黑盒(xiaoheihe.cn)SDK:**P0 只读**——扫码登录、帖子/评论/首页帖子流/@消息/用户资料查询。**不提供任何写操作**(回复评论、发帖属红线扩展 P1,未实现)。

## 环境检查

```bash
amechan-xiaoheihe help    # 查看命令与选项
which amechan-xiaoheihe   # 确认已安装
```
...
```bash
amechan-xiaoheihe feed                    # 首页帖子流(linkid/title/description)
amechan-xiaoheihe link <id>               # 帖子详情 + 评论区第 1 页(标题/正文段落/评论/翻页信息)
amechan-xiaoheihe comments <id> [page]    # 帖子评论区翻页(默认第 1 页)
amechan-xiaoheihe messages                # @消息列表(需登录;含召唤者/帖子/文本)
amechan-xiaoheihe user <id>               # 用户资料(需登录)
```
...
- `AUTH_EXPIRED` / `LOGIN_REQUIRED`:需重新 `amechan-xiaoheihe login`。
